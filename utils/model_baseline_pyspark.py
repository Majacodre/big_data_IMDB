...
def run(
    ...
    val_out: str = "submission/baseline_validation_predictions.txt",
    test_out: str = "submission/baseline_test_predictions.txt",
) -> None:
    ...
    feature_cols = [c for c in BASE_FEATURE_COLS if c in common_cols]

    if not feature_cols:
        raise ValueError("No common feature columns found across train/val/test.")

    print(f"[INFO] Using {len(feature_cols)} features for BASELINE")
    ...
    print("=" * 60)
    print("TRAINING Baseline Model — 3-fold CV over param grid")
    print("=" * 60)

    cv_model = cv.fit(train_data)
    best_model = cv_model.bestModel

    print(f"[INFO] Baseline: Best CV AUC-ROC: {max(cv_model.avgMetrics):.4f}")
    print(f"[INFO] Baseline: All CV AUC-ROC scores: {[round(m, 4) for m in cv_model.avgMetrics]}")
    ...
    print(f"[INFO] Baseline Train accuracy: {acc_eval.evaluate(train_preds):.4f}")
    print(f"[INFO] Baseline Validation accuracy: {acc_eval.evaluate(val_preds):.4f}")
    ...
    pd.DataFrame(feature_importance_data).to_csv("data/baseline_feature_importance_results.csv", index=False)
    print("[INFO] Baseline feature importances saved to data/baseline_feature_importance_results.csv")
